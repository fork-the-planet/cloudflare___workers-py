...
# Ported from workerd (https://github.com/cloudflare/workerd/tree/main/src/workerd/server/tests/python/python-rpc)
...
def assert_equal(a, b, avoid_type_check):
    ...
    # workers-py deserializes RPC'd dicts to JsDict (a dict subclass), so accept any
    # dict subclass where a dict is expected
    types_match = type(a) is type(b) or (isinstance(a, dict) and isinstance(b, dict))
    if not types_match and not avoid_type_check:
        raise ValueError(
...
class Default(WorkerEntrypoint):
    # `workerd test` only invokes a `test` handler on the default entrypoint. With
    # python_no_global_handlers (enabled by compat date >= 2025-08-14) the module-level
    # test function is not registered, so the suite is driven from this Default class.
    async def test(self):  # noqa: PLR0915
        env = self.env
        ...
        nested_none = await env.PythonRpc.identity(
            {"a": 1, "b": None, "c": {"d": None}}
        )
        ...
        # Web/API Types
        # - Response
        py_response = await env.PythonRpc.handle_response(
        ...
        py_response2 = await env.PythonRpc.handle_response(
